...
class JWTExtractorTest extends UnitSuite {
  ...
  test("That all roles are extracted") {
    ...
    roles.size should be (4)
    roles.contains("listing-test:write") should be (true)
    roles.contains("articles-test:write") should be (true)
    roles.contains("articles-staging:write") should be (true)
    roles.contains("audio-staging:write") should be (true)
  }
